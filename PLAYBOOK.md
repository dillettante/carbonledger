...
>
> 🔁 **대량 처리 시**: 쿼터 초과(429)·일시 장애(5xx)·네트워크 끊김은 **자동으로 3회까지 재시도**합니다(제공자가 알려 준 대기시간을 지킵니다). 키 오류·크기 초과처럼 다시 해도 같은 결과인 오류는 재시도하지 않고 바로 사유를 알려 줍니다. 끝내 실패한 건은 검토 대기로 남으니 나중에 재실행하면 그 건만 다시 시도됩니다.
...
- 지명(출발·도착)만 지도 API로 전송됩니다. 같은 지명은 실행 중 한 번만 조회합니다(쿼터 절약).
- 어느 지명이 어느 좌표·어느 장소로 해석됐는지는 리포트 §4-1과 엑셀 '거리산정근거' 열에 남습니다.
...
### MCP 서버로 연결하기 (선택 — 터미널 명령을 아예 안 치고 싶을 때)

에이전트에게 매번 명령을 설명하는 대신, **이 툴을 에이전트의 도구로 등록**할 수 있습니다. Claude Desktop·Claude Code 등 MCP를 지원하는 클라이언트면 됩니다.

```bash
pip install -e ".[mcp]"     # MCP SDK 추가 설치
```
...
```json
{
  "mcpServers": {
    "carbonledger": {
      "command": "carbonledger-mcp",
      "env": { "CARBONLEDGER_BACKEND": "lmstudio" }
    }
  }
}
```
- 증빙 이미지 추출에 쓸 백엔드·API 키는 위 `env`에 넣습니다(§준비물 2와 동일한 환경변수).
- 등록되는 도구 8종: `carbonledger_run`(산정 실행) · `review_status`(검토 대기 조회) · `review_merge`(교정본 병합) · `get_records`(건별 명세) · `list_factors`(계수 조회) · `inventory_template`(조직 선언 서식) · `diff`(전년 대비) · `selftest`.
- **계산 도구는 일부러 넣지 않았습니다.** 에이전트가 `활동량 × 계수`를 직접 하면 단위검사·산술 불변식·계수 출처·교정 이력이 전부 사라지기 때문에, 파이프라인 전체만 노출합니다. 교정본도 CLI와 **같은 관문**을 통과해야 반영됩니다(에이전트가 만든 JSON도 예외 없음).
- 여전히 **사람이 정해야 하는 것**은 그대로입니다 — Scope 귀속(법인차/개인차), 교정값(원증빙 확인), 조직 선언(경계·제외 사유). 에이전트가 이것들을 지어내지 않도록 서버가 안내 문구를 함께 내보내지만, 최종 확인은 사용자 몫입니다.

웹 대시보드는 여전히 의도적으로 만들지 않았습니다(설계 원칙: 최소 구성).
...
4. **건별 명세** — 파일 → 활동량 → 계수 → 배출량. 감사 추적용.
   - **4-1. 거리 산정 근거**(출장이 있을 때) — 각 지명이 **어느 좌표로 해석됐는지**와 대권거리·우회계수. 지도 API를 탄 건은 검색 1순위 결과라 동명 지점·유사 상호로 엉뚱한 곳이 잡힐 수 있습니다 — 거리만 보지 말고 해석 결과를 확인하세요.
5. **사용된 배출계수·출처** — 각 계수의 값·출처·신뢰수준·**한계**(예: "CO2만 반영", "UK 프록시").
6. **검토 대기** — 검증을 통과 못해 합계에서 빠진 건과 이유.

`report.xlsx`는 같은 내용을 5개 시트(총괄/조직선언/건별/계수목록/검토대기)로 — 총괄에는 md와 같은 **소계(Scope 1+2)** 행이 있고, 조직선언 시트 덕에 엑셀만 따로 돌아도 "무엇에 대한 숫자인지"가 남습니다. `records.json`은 재집계·프로그램 연동용 원장.
...
- **계수 × 활동량 = 배출량** 산술이 맞아야 합니다(±1%) — `factor_value`·`activity_value` 자체도 필수입니다(안 적으면 산술 검증을 우회하게 되므로)
- 필수 필드(source_file·scope)·배출량 부호도 검사합니다. **Scope 3면 `category`(1~15)도 필수**입니다(없으면 카테고리 표합과 총계가 어긋남)
- **Scope 1·2 교정본은 `factor_id`·`activity_unit`도 필수** — 카테고리 3(연료·에너지 상류) 자동 재파생이 이 두 필드로 계산됩니다. 원본 Scope 1·2 건을 교정하면 그 파생분은 **자동으로 다시 계산**되니 파생 건(`…→cat3`)을 직접 교정하지 마세요(직접 교정은 반려됩니다)
- **교정 대상은 records 또는 검토 대기에 실재하는 source_file**이어야 합니다 — 오타는 반려됩니다(빠뜨린 증빙을 새로 넣으려면 review가 아니라 입력 폴더에 넣고 재실행)
- 반려된 건은 화면과 리포트 검토 대기에 **사유와 함께 기록**되고 집계에 반영되지 않습니다
...
| "API 오류 HTTP 429 … 쿼터 소진" | 상용 API 분당 한도 초과. 자동으로 3회까지 재시도(제공자가 알려 준 대기시간 준수)했는데도 실패 | 잠시 후 재실행하면 이미 산정된 건은 그대로 두고 검토 대기 건만 처리됩니다. 대량이면 나눠 실행 |
| "이미지 N쪽이 Anthropic 상한 초과" | 고해상도 스캔(base64 10MB 초과) | 스캔 해상도를 낮추거나 이미지를 축소. PDF는 자동으로 낮은 해상도로 다시 렌더합니다 |
| "요청 전체가 상한 초과(32MB)" | 다중페이지 PDF가 통째로 전송됨 | PDF를 쪽별로 나눠 입력 |
| "API 오류 HTTP 401/403" | 키 미설정·권한 없음 | 환경변수와 제공자 콘솔의 키 권한 확인(재시도하지 않습니다 — 같은 결과라서) |
...
| 택시·선박 영수증이 검토 대기 | 계수 미등재 수단('기타') — 자동 산정은 철도·항공·버스만 | 수기 산정 후 교정본으로 반영(§3) |
| 투자(cat15)가 반려됨: "귀속계수 > 1" | 잔액(outstanding)이 기업가치(denominator)보다 큼 — 입력 오류 | 잔액·기업가치 단위(원)·값 확인 |
| 지출·통근 행이 반려됨 | 음수 금액·계수, 편도 500km 초과, 근무일 366일 초과 | 오타(부호·자릿수) 확인 후 수정 |
