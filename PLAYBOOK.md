...
3-B. [전년 대비 대조 `diff`](#3-b-전년-대비-대조--carbonledger-diff)
...
└── inventory.json        ← (선택) 조직 선언 — 우리가 누구고 어디까지 셌는지
```
...
### 1-F. inventory.json (조직 선언 — 선택이지만 강력 권장)

프로그램은 **숫자를 계산할 뿐, 그 숫자가 무엇에 대한 숫자인지는 모릅니다.** 우리 회사 이름이 뭔지, 어느 공장까지 셌는지, 자회사를 왜 뺐는지 — 전부 여러분만 압니다. 이걸 `input/inventory.json`에 적으면 리포트 맨 앞(§0)에 **그대로 옮겨 실립니다.**

`examples/input/inventory.json`을 복사해 고쳐 쓰세요:

| 칸 | 뭘 적나 |
|---|---|
| `organization` | 회사명 |
| `responsible` | 누가 작성했나(팀·담당) |
| `reporting_period` | `2026-01-01 ~ 2026-12-31` 같은 문장 |
| `consolidation_approach` | `운영통제` / `재무통제` / `지분율` 중 하나 |
| `boundary_description` | "국내 본사 1·공장 2. 해외법인 제외" 식 서술 |
| `significance_criteria` | Scope 3에서 뭘 넣고 뭘 뺄지의 기준(예: "총 Scope 3의 5% 이상") |
| `base_year` | 기준연도와 재산정 방침 |
| `exclusions` | 뺀 것과 **그 사유** (`{"item": ..., "reason": ...}` 목록) |
| `verification` | 검증 상태(`미검증`/`검증예정`/`검증완료`) |

**왜 이게 중요한가 — 실제 대기업 보고서 3사를 3개년 대조해 보니:** 배출량 숫자가 아니라 **이 선언부가 조용히 바뀌면서** 전년 대비 비교가 깨졌습니다. 한 회사는 Scope 3 카테고리 집합이, 한 회사는 포함 가스 종류가, 한 회사는 취합 범위와 기준연도가 바뀌었습니다. 숫자는 그대로 이어지는데 **의미가 달라진 것**입니다. 이 칸을 채워 두면 내년에 `carbonledger diff`(§3-B)로 대조할 때 무엇이 달라졌는지 드러납니다.

**프로그램이 안 하는 것:**
- 연결기준이 맞는지, 제외가 정당한지 **심사하지 않습니다.** 여러분 말을 여러분 말로 옮길 뿐입니다.
- 재무제표 연결범위와 대조해 "빠진 법인"을 찾아 주지 **않습니다**(회계 자료를 모릅니다).
- `verification`을 "검증완료"로 적어도, 그것은 **조직 인벤토리에 관한 선언**이지 이 프로그램이 낸 숫자가 검증받았다는 뜻이 아닙니다 — 리포트가 그 점을 함께 적습니다.

파일이 없어도 실행은 됩니다. 대신 리포트에 "조직 선언 없음"이라고 적히고, 회사명도 경계도 없는 배출량 표만 남습니다.
...
0. **조직 선언** — `inventory.json`에 적은 내용 전재(회사명·보고기간·연결기준·경계·제외 사유·검증 상태). 안 넣었으면 "선언 없음"이라고만.
...
## 3-B. 전년 대비 대조 — `carbonledger diff`

작년 산출물과 올해 산출물을 대조해 **무엇이 왜 달라졌는지**를 냅니다.

```bash
carbonledger diff 2025년/records.json 2026년/records.json --out 대조.md
```

리포트는 세 부분입니다. **순서가 중요합니다** — 배출량보다 기준을 먼저 봅니다.

1. **§1 산정 기준 변화** — 보고기간·계수판 버전·산정 카테고리 집합·사용 계수. 여기에 변화가 있으면 두 수치는 **비교 대상이 아닙니다**. 카테고리가 늘거나 줄면 총계 증감의 일부는 배출 변화가 아니라 범위 변화입니다.
2. **§2 배출량 변화** — Scope별·카테고리별 증감
3. **§3 요인분해** — `Δ배출 ≈ Δ활동량 × 종전계수 + 금번활동량 × Δ계수`. **계수 효과가 크면 감축이 아니라 산정 기준 변경**입니다. 활동량이 1000배 배수로 바뀌면 '단위 변경 의심' 표시가 붙습니다(GJ↔TJ 같은 표기 변경을 실제 증감으로 오독하는 것을 막습니다).

> **왜 이 기능이 있나** — 국내 대기업 3사의 지속가능경영보고서 3개년을 대조해 보니, 공시가 무너지는 곳은 값이 아니라 **기준의 조용한 변경**이었습니다. 한 회사는 Scope 3 산정 카테고리가 11→10→12개로 바뀌었는데 고지가 없었고, 다른 회사는 새 온실가스를 산정하며 과거를 소급하지 않아 증감의 부호가 뒤집혔습니다. 또 어느 회사도 배출 증감이 활동량 때문인지 계수 때문인지 밝히지 않았습니다. 이 명령은 **자기 산출물에 대해 그 일을 자동으로** 합니다.
