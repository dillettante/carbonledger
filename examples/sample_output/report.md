...
| Scope 3 (기타 간접) | 465.689 tCO2eq (465,688.6 kg) | 99.93% |
| **총계 (Scope 1+2+3)** | **466.026 tCO2eq (466,026.1 kg)** | 100% |
...
| 3 | 연료 및 에너지 관련 활동 (Scope 1·2 미포함분) | 측정 (0.0%) | 0.055 tCO2eq (55.5 kg) | 자동 파생: 산정된 Scope 1 연료·Scope 2 전력 활동량 × WTT/T&D 계수(wtt_*·td_*). 별도 입력 불필요 |
| 4 | 업스트림 운송 및 유통 | 측정 (0.4%) | 1.760 tCO2eq (1,760.0 kg) | 물량기반(톤·km × 운송수단 계수) 또는 운송비 지출기반 |
| 5 | 사업장 발생 폐기물 | 측정 (0.2%) | 1.048 tCO2eq (1,048.1 kg) | 물량기반(폐기물 종류·처리방법별 발생량 × 계수) |
| 6 | 출장 | 측정 (0.0%) | 0.011 tCO2eq (10.6 kg) | 거리기반: 증빙(승차권·항공권·숙박 영수증)에서 AI 추출 → 구간 거리 × 수단별 인·km 계수. input/travel/ 폴더 입력 |
| 7 | 직원 통근 | 측정 (0.8%) | 3.857 tCO2eq (3,856.7 kg) | 거리기반: 통근 설문 CSV(수단·편도거리·연간 근무일) × 수단별 계수. commute.csv 입력 |
| 8 | 업스트림 임차 자산 | 측정 (1.0%) | 4.590 tCO2eq (4,590.3 kg) | 임차 자산의 에너지 사용량 × Scope 1·2 계수 |
| 9 | 다운스트림 운송 및 유통 | 측정 (0.3%) | 1.427 tCO2eq (1,427.4 kg) | 물량기반(톤·km × 운송수단 계수) |
...
| 12 | 판매 제품의 폐기 | 측정 (0.0%) | 0.027 tCO2eq (27.0 kg) | 물량기반(제품·포장재 재질별 폐기 계수 × 판매 물량) |
...
| commute.csv#1 | S3 | 통근 지하철 | 7920.0 passenger-km | `commute_subway` |  | 122.681 |
| commute.csv#2 | S3 | 통근 시내버스 | 3960.0 passenger-km | `commute_bus_local` |  | 401.98 |
| commute.csv#3 | S3 | 통근 승용차(휘발유) | 11000.0 km | `commute_car_petrol` |  | 1776.72 |
| commute.csv#4 | S3 | 통근 승용차(하이브리드) | 12000.0 km | `commute_car_hybrid` |  | 1555.32 |
...
| cat12_eol.csv#1 | S3 | 판매제품 폐기: 판매제품 포장재 폐기(플라스틱 3t) | 3.0 tonne | `waste_plastic_landfill` |  | 27.021 |
...
| cat4_upstream_transport.csv#1 | S3 | 업스트림 운송·유통: 원자재 트럭 운송(50t×300km) | 15000.0 tonne-km | `freight_hgv` |  | 1553.4 |
| cat4_upstream_transport.csv#2 | S3 | 업스트림 운송·유통: 부품 철도 운송(20t×400km) | 8000.0 tonne-km | `freight_rail` |  | 206.64 |
| cat5_waste.csv#1 | S3 | 사업장 폐기물: 사무실 혼합폐기물 매립 2t | 2.0 tonne | `waste_mixed_landfill` |  | 1041.16 |
| cat5_waste.csv#2 | S3 | 사업장 폐기물: 폐지 재활용 1.5t | 1.5 tonne | `waste_recycling` |  | 6.98 |
...
| cat9_downstream_transport.csv#1 | S3 | 다운스트림 운송·유통: 제품 배송 트럭(30t×200km) | 6000.0 tonne-km | `freight_hgv` |  | 621.36 |
...
| 전기_한전_202605.png→cat3 | S3 | 연료·에너지 상류(WTT/T&D): 전력 사용 | 500.0 kWh | `wtt_electricity_kr` |  | 18.41 |
| 전기_한전_202605.png→cat3 | S3 | 연료·에너지 상류(WTT/T&D): 전력 사용 | 500.0 kWh | `td_electricity_kr` |  | 6.495 |

### 4-1. 거리 산정 근거 (출장 — 지명→좌표 해석)

| 파일 | 출발(해석 결과) | 도착(해석 결과) | 대권거리 | 우회계수 | 적용거리 |
|---|---|---|---|---|---|
| ktx_seoul_busan_synthetic.png | 서울 → 내장 역좌표 (37.5547, 126.9707) | 부산 → 내장 역좌표 (35.1151, 129.0403) | 328.5 km | ×1.2 | 394.3 km |

> 좌표 출처가 지도 API(Kakao·Naver)면 **검색 1순위 결과**다 — 동명 지점·유사 상호로 엉뚱한 곳이 잡힐 수 있으니 해석 결과를 확인할 것. 거리는 대권(직선) 근사이며 철도·버스는 우회계수로 실노선을 근사한다.
...
| `commute_subway` | 0.01549 | kgCO2eq/passenger-km | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, London Underground (한국 지하철 공식 인·km 계수 부재로 대용) | 한국 전력믹스가 영국보다 탄소집약적이라 과소 가능성. 서울교통공사 지속가능경영보고서 실측 확보 시 교체 예정. 2026판에서 -44%(vs 2024) — 영국 그리드 탈탄소 + COVID 이후 수송량 갱신(major changes #53). UK 프록시 한계가 더 커졌으니 주의. |
| `commute_bus_local` | 0.10151 | kgCO2eq/passenger-km | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Average local bus |  |
| `commute_car_petrol` | 0.16152 | kgCO2eq/km | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Average car/Petrol | 차량당(vehicle·km). 1인 탑승 가정이면 그대로, 카풀 시 탑승인원으로 나눔. |
| `commute_car_hybrid` | 0.12961 | kgCO2eq/km | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Average car/Hybrid | 차량당. |
...
| `waste_plastic_landfill` | 9.00687 | kgCO2eq/tonne | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Waste disposal (Plastics: average, Landfill) | 카테고리 5·12 공용. |
...
| `freight_hgv` | 0.10356 | kgCO2eq/tonne-km | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Freighting goods (Average non-refrigerated HGVs, average laden, diesel) | 카테고리 4(업스트림)·9(다운스트림) 공용. 활동량=톤×km. 한국 완제 tonne-km 계수 부재로 DEFRA 대체. 2026판은 HGV를 냉장/비냉장으로 분리 — 본 계수는 비냉장 평균(2024 'All HGVs'의 후속). 냉장운송이 크면 별도 계수(사용자 입력) 검토. |
| `freight_rail` | 0.02583 | kgCO2eq/tonne-km | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Freighting goods (Rail, freight train) | 활동량=톤×km(사용자가 선곱해 입력). 카테고리 4·9 공용. |
| `waste_mixed_landfill` | 520.58023 | kgCO2eq/tonne | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Waste disposal (Commercial and industrial waste, Landfill) | 카테고리 5(사업장 폐기물)·12(제품 폐기) 공용. 한국 환경부 완제 tonne당 계수 부재로 DEFRA 대체. |
| `waste_recycling` | 4.65358 | kgCO2eq/tonne | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Waste disposal (Commercial and industrial waste, Closed-loop recycling) | ⚠️ 처리공정만 반영(재질 무관 동일값). 재활용의 회피배출(avoided) 크레딧은 미포함 — 별도 방법론. 카테고리 5·12 공용. |
| `freight_ship_container` | 0.01612 | kgCO2eq/tonne-km | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Freighting goods (Cargo ship, container, average) | 활동량=톤×km(사용자가 선곱해 입력). 카테고리 4·9 공용. |
| `wtt_diesel` | 0.61101 | kgCO2eq/L | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, WTT- fuels (diesel average) | Scope 1 경유 연소(fuel_diesel)에서 자동 파생. |
| `wtt_electricity_kr` | 0.03682 | kgCO2eq/kWh | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, WTT- UK electricity (generation) — 한국 값 부재로 UK 프록시 | ⚠️ UK 전력 상류 프록시 — 한국 발전믹스와 다르다. 규제용 부적합, 자사 값 확보 시 교체. Scope 2 전력에서 자동 파생. |
| `td_electricity_kr` | 0.01299 | kgCO2eq/kWh | 해외정부공식 | 2026 | AR5(DEFRA) | DEFRA/DESNZ 2026, Transmission and distribution — 한국 값 부재로 UK 프록시 | ⚠️ UK 송배전 손실 프록시 — 한국 손실률(~3.5%)·발전계수와 다르다. 규제용 부적합. Scope 2 전력에서 자동 파생. |
